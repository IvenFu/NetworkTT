...
void FitSubMask(int num_mask_bytes, int num_sub_mask_bytes, int num_rows,
                const HPR_UINT8* sub_mask, HPR_UINT8* packet_mask) {
  if (num_mask_bytes == num_sub_mask_bytes) {
	  
	  //第一部分(24,24)矩阵，示例如下
	  //@  A
	  //  xxxxxx    yyyxxx      yyyxxx      
	  //  xxxxxx	yyyxxx	->	yyyxxx 
	  //    ..        ..          ..       
	  //  xxxxxx				xxxxxx	
...
void ShiftFitSubMask(int num_mask_bytes, int res_mask_bytes,
                     int num_column_shift, int end_row, const HPR_UINT8* sub_mask,
                     HPR_UINT8* packet_mask) {

 //第一部分(8,8)矩阵，第二部分(8,2)矩阵，示例如下
 //@  C
 //  yxxxxx   tx        yxxxxx      
 //  yxxxxx	  tx	->	yxxxxx
 //	   ..                 ..
 ...
 //num_column_shift = 8    end_row = 10    num_mask_bytes = 6   res_mask_bytes= 2


 //第一部分(8,8)矩阵，第二部分(24,2)矩阵,  y和t有效
 //@  D
 //  yxxxxx   tttxxx        yxxxxx      
 //  yxxxxx	  tttxxx	->	yxxxxx
 //    ..                     ..
 //  xxxxxx					xtttxx
 //  xxxxxx					xtttxx

 //num_column_shift = 8    end_row = 10    num_mask_bytes = 6   res_mask_bytes= 6 
  ...
  for (int i = num_column_shift; i < end_row; ++i) {
    ...
    //@ num_byte_shifts 不等于0， res_mask_bytes等于6，会不会越界？
    int pkt_mask_idx =
    ...
    HPR_UINT8 shift_right_curr_byte = 0;
    HPR_UINT8 shift_left_prev_byte = 0;
    HPR_UINT8 comb_new_byte = 0;
...
// |fec_mask_type|.
const HPR_UINT8*** PacketMaskTable::InitMaskTable(FecMaskType fec_mask_type) {
  switch (fec_mask_type) {
...
void RemainingPacketProtection(int num_media_packets, int num_fec_remaining,
                               ...
                               ProtectionMode mode, HPR_UINT8* packet_mask,
  ...
  if (mode == kModeNoOverlap) 
  {
	...
	//@ 如果num_fec_for_imp_packets 不等于 num_imp_packets 呢？？
...
void ImportantPacketProtection(int num_fec_for_imp_packets, int num_imp_packets,
                               int num_mask_bytes, HPR_UINT8* packet_mask,
                               const PacketMaskTable& mask_table) {
  ...
  const HPR_UINT8* packet_mask_sub_1 = mask_table.fec_packet_mask_table()[
...
int SetProtectionAllocation(int num_media_packets, int num_fec_packets,
  ...
  //@ import 数据部分，FEC保护度是 255，即1比1，且不超过0.5倍的原始FEC个数
  float alloc_par = 0.5;
...
void GeneratePacketMasks(int num_media_packets, int num_fec_packets,
                         int num_imp_packets, HPR_BOOL use_unequal_protection,
                         const PacketMaskTable& mask_table,
                         HPR_UINT8* packet_mask) {
