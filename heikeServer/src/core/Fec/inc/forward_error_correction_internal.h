...
#ifndef _FORWARD_ERROR_CORRECTION_INTERNAL_H_
#define _FORWARD_ERROR_CORRECTION_INTERNAL_H_
...
class PacketMaskTable {
 public:
  ...
  const HPR_UINT8*** fec_packet_mask_table() const {
    return fec_packet_mask_table_;
  }

 private:
  FecMaskType InitMaskType(FecMaskType fec_mask_type, int num_media_packets);
  const HPR_UINT8*** InitMaskTable(FecMaskType fec_mask_type_);
  const FecMaskType fec_mask_type_;
  const HPR_UINT8*** fec_packet_mask_table_;
};
